...
function CylinderGlossary() {
  ...
  const searchResults = useMemo(() => {
    ...
    results.sort((a, b) => {
      ...
      const priorityKeywords = [
        "mortise cylinder",
        "rim cylinder",
        "bored locks cylinder",
      ];

      let aPriority = priorityKeywords.findIndex((keyword) =>
        aName.includes(keyword)
      );
      let bPriority = priorityKeywords.findIndex((keyword) =>
  ...
  const displayedCylinders = useMemo(() => {
    ...
    return glossaryData.cylinderTypes.filter(
      (c) => c.id === selectedCylinderType
    );
    ...
    <div className="glossary-container">
      ...
      {isInitialState ? (
        <div className="glossary-initial-view">
          ...
          <p>
            Use the search bar above to find a specific part, or select a
            cylinder type to browse its components.
          </p>
        </div>
        ...
        displayedCylinders.map((cylinder) => (
          <div
            key={cylinder.id}
            className={`cylinder-section ${
              openCylinderId === cylinder.id ? "open" : ""
            }`}
          >
            <button
              className="cylinder-section-header"
              onClick={() => handleToggleCylinder(cylinder.id)}
            >
              <img
                src={cylinder.imageUrl}
                alt={`${cylinder.name} diagram`}
